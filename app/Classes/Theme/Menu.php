...
class Menu
{
     ...
     * @var boolean
     */
    private static $_activeAlreadyBeingShown = false;
    ...
    public static function renderVerMenu($item, $parent = null, $rec = 0, $singleItem = false)
    {
        ...
        if (isset($item['permissions'])) {
            if (!\Auth::user()->checkPermissions($item['permissions'])) {
                return '';
            }
        }
        ...
        if (isset($item['separator'])) {
            ...
            if (isset($item['parent'])) {
                ...
                if (isset($item['page'])) {
                    if (preg_match('/([a-z\-]+\.[a-z]+)\|(\{.*?\})/', $item['page'], $match)) {
                        $url = route($match[1], json_decode($match[2], true));
                    }
                    elseif (preg_match('/[a-z]+\.[a-z]+/', $item['page'])) {
                        $url = route($item['page']);
                    } else {
                        if (in_array($item['page'], ['dashboard'])) {
                            $url = route($item['page']);
                        } else {
                            $url = url($item['page']);
                        }
        ...
        if (isset($item['separator'])) {
            ...
            if (isset($item['code'])) {
                ...
                if (isset($item['heading']) == false) {
                    ...
                    if (isset($item['page'])) {
                        if (preg_match('/[a-z]+\.[a-z]+/', $item['page']) && !preg_match('/^http/i', $item['page'])) {
                            $url = route($item['page']);
    ...
    public static function isActiveVerMenuItem($item, $page, $rec = 0)
    {
        ...
        if (isset($item['page']) && preg_match('/([a-z\-]+\.[a-z]+)\|(\{.*?\})/si', $item['page'],$match)) {
            $requestUrl = preg_replace_callback('/^https?\:\/\/[^\/]+\/{1}(.*?)/si', function($match) {
                return $match[1];
            }, route($match[1],json_decode($match[2],true)));

            // checking users page on some types like 'fans' to set it up active
            $decodeParams = json_decode($match[2],true);

            if (preg_match('|^'.$requestUrl.'$|i', $page)) {
                return true;
            }
        }
        elseif (isset($item['page']) && preg_match('/[a-z\-]+\.[a-z\-]+$/si', $item['page']) && !self::$_activeAlreadyBeingShown) {
            $requestUrl = preg_replace_callback('/^https?\:\/\/[^\/]+\/{1}(.*?)/si', function($match) {
                return $match[1];
            }, route($item['page']));
            if (preg_match('/\/\d+/', $page) && preg_match('|^'.$requestUrl.'/\d+|i', $page)) {
                return true;
            }
            elseif (!preg_match('/\/\d+/', $page)) {
                if ($requestUrl == $page) {
                    return true;
                }
            }
        } else
        if (isset($item['page']) && $item['page'] == $page && !self::$_activeAlreadyBeingShown) {
            return true;
        }
